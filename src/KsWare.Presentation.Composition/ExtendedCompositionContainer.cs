...
namespace KsWare.Presentation.Composition {
	...
	public class ExtendedCompositionContainer : CompositionContainer {
		...
		public ExtendedCompositionContainer(ComposablePartCatalog catalog, params ExportProvider[] providers) : base(
			catalog, providers) {
		}
		...
		protected override IEnumerable<Export> GetExportsCore(ImportDefinition definition, AtomicComposition atomicComposition) {
			...
			var ind = new string(' ', level * 4);
			var indContractName = $"{ind} {definition.ContractName}";
			Debug.WriteLine($"Container.GetExportsCore {indContractName,-64} {(definition.IsPrerequisite ? "Prerequisite" : ""),-12} => Success: {entry.Success,-5}, Exports: {exports.Count()}");
